...
class extendedKFBatt():
    def __init__(self, N, filename):
        ...
        self.storeDF = pd.DataFrame({'SOC': self.socStore, 'Irc1': self.Irc1Store, 'Irc2': self.Irc2Store, 'socSigma': self.socSigmaStore, 'Irc1Sigma': self.Irc1SigmaStore, 'Irc2Sigma': self.Irc2SigmaStore, 'I': self.IStore, 'V': self.VStore, 'yHat': self.yHatStore, 'VOCV': self.VOCVStore, 'Innovation': self.innovationStore, 'dOCVSOC': self.dOCVSOCStore, 'yHatKF': self.yHatKFStore})
    ...
    def iterKF(self):
        for k in range(self.N):
            ...
            Bhat = np.array([np.array([-self.dt * self.eta/(3600 * self.capacityOCV)]), 1-self.RC1, 1-self.RC2]).T
            Chat = np.array([np.array([dOCVSOC]), -self.r1, self.r2]).T
            Dhat = np.array([1])

            '''EKF step 1: state estimate prediction update'''
            self.xHat[0] = self.xHat[0] - self.dt * I * self.eta/(3600 * self.capacityOCV)
            ...
            self.sigmaX = np.dot(np.dot(Ahat, self.sigmaX), Ahat.T) + np.dot(np.dot(Bhat, self.sigmaW), Bhat.T)

            '''EKF step 1c: predict system output'''
            self.yHat = OCV - self.r1 * self.xHat[1] - self.r2 * self.xHat[2] - self.r0 * I

            '''EKF step 2a: compute kalman gain'''
            sigmaY = np.dot(np.dot(Chat, self.sigmaX), Chat.T) + np.dot(np.dot(Dhat, self.sigmaV), Dhat.T)
            ...
            self.sigmaX = (self.sigmaX + np.transpose(self.sigmaX) + HH + np.transpose(HH))/4
        ...
        self.storeDF = pd.DataFrame({'SOC': self.socStore, 
        'Irc1': self.Irc1Store, 'Irc2': self.Irc2Store, 'socSigma': self.socSigmaStore, 'Irc1Sigma': self.Irc1SigmaStore, 'Irc2Sigma': self.Irc2SigmaStore, 'I': self.IStore, 'V': self.VStore, 'yHat': self.yHatStore, 'VOCV': self.VOCVStore, 'Innovation': self.innovationStore, 'dOCVSOC': self.dOCVSOCStore, 'yHatKF': self.yHatKFStore})
